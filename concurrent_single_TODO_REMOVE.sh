...
while [  $COUNT -lt $NUM ]; do
	...
	#run in parralel
	./testrunner.sh -r -j -a -sTests2 -f./tests/results.scon$COUNT -Gpropfile=settings.scon$COUNT.properties -t./tests/global.settings.xml ./tests/appudo.scon$COUNT.xml &
	pids="$pids $!"
	COUNT=$(($COUNT+1))
done
#wait
